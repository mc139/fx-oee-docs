...
1. After every `submit(order)` produces trades, the executed trade prices are fed to
   `CircuitBreaker.onTrade`, **outside the
   ...
   `SpeedMatchingService.java:457`.
2. If the breaker is disabled (`fx.circuit-breaker.enabled=false`) or either argument is null,
   `onTrade` returns immediately and does nothing (`CircuitBreaker.java:59`).
...
4. If `deviation > threshold` **and** the pair is currently `OPEN`, the breaker:
   - sets the pair to `HALTED` via `TradingStatusService`;
   ...
   - if Kafka is enabled, publishes a `TradingHaltedEvent` (pair, deviation, threshold, timestamp)
     via the optionally-injected `OrderEventProducer` (`CircuitBreaker.java:73`).
...
and threshold `0.005` (0.5%). The k8s `configmap` ships
...
(`CircuitBreaker.java:48`) are `enabled=true`
...
**Resume halted pairs** (`CircuitBreakerController`):
