...
**Status:** Accepted · its Kafka-as-WAL follow-up was superseded by [ADR 0007](0007-aeron-archive-wal-questdb-tape.md) · refines [ADR 0002](0002-in-memory-authoritative-engine.md) · builds on [ADR 0005](0005-disruptor-adoption.md)

> **Shipped, off by default.** `EngineSnapshotter` + `bootstrap.SnapshotStore` + bounded `recoverFromLog`
> exist behind `fxoee.recovery.snapshots.enabled` (needs `kafka.enabled=true`). The `restore` primitive
> is on both engines and unit-tested. The headline _Deferred_ (full Kafka-as-WAL durability with an
> engine-stamped sequence) was **not** built on Kafka: [ADR 0007](0007-aeron-archive-wal-questdb-tape.md)
> delivered the exact engine-stamped sequence as the Aeron Archive recording position instead, retiring
> the Kafka durability path this ADR's follow-up assumed. This ADR stands as the bounded-restart
> foundation and the analysis of why the cut was opportunistic on Kafka.

## Context

[ADR 0002](0002-in-memory-authoritative-engine.md) fixed the system's shape: the in-memory matching
engine is the source of truth, and Postgres + Kafka are projections rebuilt from a durable log. An
earlier throughput-redesign effort (phases 0-3) then made the persistence tail keep up with the
[speed engine](0005-disruptor-adoption.md):
...
That redesign's phase 4 also aimed higher: move durability fully onto Kafka (the engine publishes
straight to a replicated WAL; no synchronous `trade_events` INSERT on the worker thread). That shift is
the highest-risk change in it - it rewrites the durability contract and the recovery path that is
...
## Deferred - full Kafka-as-WAL durability (now superseded by ADR 0007)

This ADR deliberately stopped short of the original throughput-redesign phase-4 goal (drop the synchronous
`trade_events` append; durability via a replicated WAL). Reaching it needed, in order:
...
2. a **whole-event WAL** for replay - the per-account `trades.byaccount` legs cannot be cheaply
   reassembled into ordered trades, so the WAL needs a whole-trade record;
3. **load validation** of retention vs snapshot interval and the crash/replay cut.

[ADR 0007](0007-aeron-archive-wal-questdb-tape.md) **delivered all three, but off Kafka**: it replaced
the Kafka durability path with an embedded Aeron Archive whose **recording position is the
engine-stamped sequence** (item 1, making the cut exact and removing the opportunistic guard), used a
**whole-trade SBE `FillRecord`** (item 2), and validated replay with an embedded driver + Archive (item
3, `SpeedWalCrossRestartRecoveryTest`). So the Kafka-as-WAL variant assumed here was not built; this
ADR's lasting contribution is the `restore` primitive (reused verbatim by ADR 0007's
`Snapshotter.recover`) and the bounded-restart shape. Back to the [ADR index](README.md).
